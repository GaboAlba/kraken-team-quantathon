...
\author{Kraken Team --- Gabriel Alba Romero, Juan C.\ Lara, Jonathan Gomez Barrios}
\date{\today}
...
deterministic. The kernel is backend-agnostic: it compiles
...
Initially, a small smoke test was run to ensure that with a small graph we would have a solution that makes sense. The result from this small run was the graph detailed in Figure 5 \ref{fig:qaoa-partition}, which for this small dataset is the best cut possible. This signal indicates that the QAOA optimizer implemented can find good solutions to these types of issues. 
...
Both Figure \ref{fig:res-box} and Figure \ref{fig:res-dist} show a lot of different aspects of the results achieved. In the first place we can observe the dramatic difference in the distribution of results. As seen, both QAOA algorithms have a very broad distribution which indicates that the results from this algorithm are not as consistent as the classical algorithms. This is most likely due to noise in the model. 
...
Finally, in terms of the convergence of the algorithm Figure \ref{fig:res-conv} shows how the approximation ratio behaved as the optimization iterations were running. In this graph it is easy to see that when p = 3, the mean of the results definitely sits higher than when p = 1, confirming the expected behavior that with more layers, it is able to find better solutions. Even then, the standard deviation is larger for the 3 layer solution indicating that the introduction of more layers also adds more noise to the solution. What this means is that there is most likely a divergence point where noise takes over and the standard deviation becomes unacceptably large.
...
\begin{enumerate}
    \item The weight calculation for th transmission lines should be fine-tuned to better represent the importance of a line
    \item The penalty coefficients in the QUBO calculation must also be fine tuned further to be able to get better approximation ratios.
    \item The implementation of a weighted AND directed QUBO representation may provide even beter solutions since it may be able to take into account transmission lines direction into the weight calculation.
    \item Use different variations of the QAOA optimizer is also worth exploring such as warm-start or recursive QAOA may present better performance.
    \item Noise reduction algorithms could also increase the performance of the optimizer
    \item Error Correction or Error Detection algorithms may also help in achieving a better performance with the tradeoff of considerably longer running times for the algorithm.
\end{enumerate}
